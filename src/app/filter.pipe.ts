...
export class FilterPipe implements PipeTransform {
  transform(value: any[], searchTerm: string): any {
    ...
    for (let i = 0; i < value.length; i++) {
      ...
      let createdBy: string = value[i].createdBy;
      let updatedBy: string = value[i].updatedBy;
      if (name.toLowerCase().indexOf(searchTerm.toLowerCase()) > -1) {
        ...
        createdBy.toLowerCase().indexOf(searchTerm.toLowerCase()) > -1
      ) {
        EmpArray.push(value[i]);
      } else if (
        updatedBy.toLowerCase().indexOf(searchTerm.toLowerCase()) > -1
      ) {
        EmpArray.push(value[i]);
      }
    }
